...
export function runShellScript<T>(
  script: string,
  options?: ScriptOptions,
): { data: Promise<T | string>; sendMessage: (msg: string) => void } {
  return runScript(script, { ...options, language: "Shell" });
}

export function runSwift<T>(
  script: string,
  options?: ScriptOptions,
): { data: Promise<T | string>; sendMessage: (msg: string) => void } {
  const leadingArgs = options?.leadingArgs || [];
  if (!script.startsWith("/")) {
    if (leadingArgs.length === 0 || leadingArgs.indexOf("-e") !== leadingArgs.length - 1) {
      leadingArgs.push("-e");
    }
  }
  return runScript(script, { ...options, command: "swift", leadingArgs });
}
...
export function runPython<T>(
  ...
  const leadingArgs = options?.leadingArgs || [];
  if (!script.startsWith("/")) {
    if (leadingArgs.length === 0 || leadingArgs.indexOf("-c") !== leadingArgs.length - 1) {
      leadingArgs.push("-c");
    }
  }
  return runScript(script, { ...options, command: "python", leadingArgs });
}

export function runPerl<T>(
  script: string,
  options?: ScriptOptions,
): { data: Promise<T | string>; sendMessage: (msg: string) => void } {
  const leadingArgs = options?.leadingArgs || [];
  ...
  return runScript(script, { ...options, command: "perl", leadingArgs });
}

export function runRuby<T>(
  script: string,
  options?: ScriptOptions,
): { data: Promise<T | string>; sendMessage: (msg: string) => void } {
  const leadingArgs = options?.leadingArgs || [];
  ...
  return runScript(script, { ...options, command: "ruby", leadingArgs });
}

export function runLua<T>(
  script: string,
  options?: ScriptOptions,
): { data: Promise<T | string>; sendMessage: (msg: string) => void } {
  const leadingArgs = options?.leadingArgs || [];
  ...
  return runScript(script, { ...options, command: "lua", leadingArgs });
}